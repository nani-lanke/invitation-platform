...
(function (window, document) {
  ...
  function toServer(state, payment) {
    /* `data` not `payload`: the module already has a function named
       payload(), and a local `var payload` would shadow it with undefined
       the instant this function ran — killing every publish, paid or not. */
    ...
    if (payment) {
      if (payment.razorpay_order_id) data.razorpay_order_id = payment.razorpay_order_id;
      if (payment.razorpay_payment_id) data.razorpay_payment_id = payment.razorpay_payment_id;
      if (payment.razorpay_signature) data.razorpay_signature = payment.razorpay_signature;
    }

    console.log('[publish] request to /api/publish', {
      orderId: payment && payment.razorpay_order_id,
      paymentId: payment && payment.razorpay_payment_id,
      hasSignature: !!(payment && payment.razorpay_signature)
    });
    ...
    return fetch('api/publish', {
      ...
      console.log('[publish] /api/publish responded with', res.status);
